...
def create_post(title: str, content: str, status: str = 'draft') -> str:
    ...
    try:
        ...
        if e.response.status_code == 401:
            return "X Authentication failed. Please check your bearer token."
        elif e.response.status_code == 403:
            return "X Permission denied. You don't have permission to create posts."
        else:
            return f"X HTTP Error {e.response.status_code}: {e.response.text}"
    except requests.exceptions.RequestException as e:
        return f"X Network error creating post: {str(e)}"
    except Exception as e:
        return f"X Unexpected error creating post: {str(e)}"
...
def get_site_info() -> str:
    ...
    try:
        ...
        return f"X Error getting site info: {str(e)}"
